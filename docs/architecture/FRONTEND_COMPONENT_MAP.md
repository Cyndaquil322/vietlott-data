...
| ID Thẻ Div | Tên View trên Menu | Chức năng chính |
|---|---|---|
| `view-consensus-content` | **⭐ Tổng Hợp Dự Đoán** | Khuyến nghị Quản trị Vốn Kelly (CCS Score 3 cấp độ), Tối ưu danh mục vé Markowitz & phân tán cụm Louvain, Bảng đấu sĩ đối soát 100 kỳ walk-forward cho 8 mô hình độc lập + Consensus, Bảng trọng số Top 15 bóng, Ma trận giải trình toán học (Explainable AI), Bản đồ lực hút liên kỳ A -> B, Bộ 7 số tối ưu Pareto, Dàn bọc lót Covering C(v, k, t) và Bộ vé đề xuất chuẩn SEI |
...
| **`consensus_ensemble.js`** | **37** | Trí tuệ đám đông Consensus Hub (xếp hạng Top 15 bóng theo 8 mô hình độc lập), widget Khuyến nghị Quản trị Vốn Kelly (`renderConsensusBankrollAdvisory`), Tối ưu danh mục vé Markowitz & phân tán cụm Louvain (`renderConsensusGoldenMarkowitz`), widget Bản đồ lực hút liên kỳ $A \to B$, widget Bộ Lọc Đào Thải Bóng Chết (`renderConsensusEliminationAnalytics`), widget Dàn Bọc Lót Bạch Thủ Chốt (`renderConsensusBankerWheeling`), bộ sinh số vàng Ensemble gieo mầm cố định theo mã kỳ quay (`Draw ID Seed`), trích xuất **Bộ 7 Số Tối Ưu Toán Học (Bao 7 Pareto)** từ Dàn Hạt Nhân kèm thẻ đối soát 100 kỳ Walk-Forward, bảng đối soát 100 kỳ chi tiết, đồng hồ đếm ngược trực tiếp (`updateCountdown`), soi cầu bóng đặc biệt Power 6/55 và Power 5/35. | `createMulberry32`, `stringToSeedHash` |
| **`notebook_bao7.js`** | **26** | Quản lý sổ tay vé cá nhân (Lưu `localStorage`, hợp nhất vé Git, đồng bộ link 1-chạm Base64 `?sync=`, sao lưu/phục hồi file JSON), đồng bộ vé tối ưu Pareto từ Dàn Hạt Nhân vào chiến lược Bao 7 (Bao 6 với 5/35), bảng đối soát 100 kỳ walk-forward lãi/lỗ và bộ tạo cú pháp tin nhắn SMS 9969 chuẩn Vietlott. | `createMulberry32`, `stringToSeedHash` |
